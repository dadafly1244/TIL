...
const navWidth = ref(260) //기본 초기화 값을 넣어준 반응형 데이터로 만들어줌 
const nav: Ref<HTMLElement | null> = ref(null)
const resizeHandle: Ref<HTMLDivElement | null> = ref(null)

workspaceStore.readWorkspaces() //create는 컴포지션api에서는 없는 라이프사이클
onMounted(() => { // mount 된 이후에는 onMounted의 콜백으로 넘겨주기! 
  resizeInit()
})
function resizeInit() {
